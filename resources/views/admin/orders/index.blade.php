    ...
    <script>
        function imprimirEtiqueta(e) {
            $.ajax({
                type: 'get',
                data: {
                    idOrdenRetiro: $(e).data('idoca'),
                   // nroEnvio: $('#nroEnvio').val(),
                    //logisticaInversa: false,//$('#log_inv').prop("checked"),
                },
                url: '{{route('envios.etiqueta_envio_oca')}}?',//'../shippings/etiqueta_envio_oca',
                dataType: 'json',
                success: function(response) {
                    $("#imprimiretiqueta button").removeAttr("disabled");
                    if(response.success && response.results != "") {
                        try {
                            window.atob(response.results);
                            let linkSource = `data:application/pdf;base64,${response.results}`;
                            let downloadLink = document.createElement("a");
                            let fileName = "etiqueta-"+$('#id_orden_retiro').val()+".pdf";
                            downloadLink.href = linkSource;
                            downloadLink.download = fileName;
                            downloadLink.click();
                            //$("#mensaje").html("");
                            //$('#imprimiretiqueta').modal('hide');
                        } catch(e) {
                            $("#mensaje").html("<p style='color: red'>" + response.results + "</p>");
                        }
                    } else {
                        $("#mensaje").html("<p style='color: red'>" + response.results + "</p>");
                    }
        ...
        function imprimirEtiquetaPickit(e) {

            let transaction = $(e).data('idtrasaccion');
         

            $.ajax({
                type: 'GET',
               
                url: 'envios/etiqueta_envio_pickit/'+transaction,
                dataType: 'json',
                success: function(response) {
                    $("#imprimiretiqueta button").removeAttr("disabled");
                    if(response.labels != "") {
                        try {
                            let linkSource = `${response.labels.url}`;
