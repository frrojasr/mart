    ...
    public function ajax()
    {
        ...
        return datatables()
            ...
            ->editColumn('customer', function ($orders) {
                ...
                ->addColumn('orden_oca', function ($orders) {

                    $view = '<a title="etiqueta oca" onclick="imprimirEtiqueta(this)" data-idoca = "'.$orders->orden_oca.'" href="#">'.$orders->orden_oca.'</a>';

                    return $view;
                    //return $orders->orden_oca;
            })
            ->addColumn('orden_pickit', function ($orders) {

                $transaccion='';

                $pickit = Pickit::where('order_id',$orders->id)->first();
                if ($pickit) {
                    if ($pickit->transaction_id){
                        $transaccion=$pickit->transaction_id;
                    }
                }
                    
                $view = '<a title="etiqueta pickit" onclick="imprimirEtiquetaPickit(this)" data-idtrasaccion= "'.$transaccion.'" href="#">'.$transaccion.'</a>';

                return $view;
                //return $orders->orden_oca;
        })

//'. route('envios.etiqueta_envio_oca', ['idOrdenRetiro' => $orders->orden_oca]).'
            ...
            ->rawColumns(['customer', 'total', 'status', 'created_at', 'reference', 'action', 'payment_status', 'vendor','orden_oca','orden_pickit'])
            ->make(true);
    }
    ...
    */
    public function html()
    {
        return $this->builder()
            ...
            ->addColumn(['data' => 'orden_oca', 'name' => 'Oca', 'title' => __('OCA'), 'orderable' => false, 'searchable' => false])
           
            ->addColumn(['data' => 'orden_pickit', 'name' => 'Pickit', 'title' => __('Pickit'), 'orderable' => false, 'searchable' => false])
